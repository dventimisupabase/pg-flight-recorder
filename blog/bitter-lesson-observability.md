...
**2. Expose raw data, not just summaries.** Dashboards are still useful for humans, but make sure the underlying data is accessible for programmatic analysis. An `export_json()` function might be more valuable than a pretty graph.
...
*This post emerged from a conversation about [pg-flight-recorder](https://github.com/dventimisupabase/pg-flight-recorder), a PostgreSQL monitoring extension designed around comprehensive telemetry capture. The `export_json()` function mentioned above is a real feature—because sometimes the best interface for a monitoring tool is a blob of structured data and an AI that knows what to do with it.*
